...
셋째, 정산 비용. 원화 스테이블코인을 ERC-20 토큰으로 쓰면 급여 자산과 가스 자산이 둘로 갈라진다. 직원 수백 명에게 한 건씩 보내면 트랜잭션 수와 가스가 인원에 비례하고, 직원은 받은 돈을 꺼내기 위해 가스용 자산을 따로 확보해야 하며, 지급 컨트랙트는 allowance와 transferFrom 계층을 거쳐야 한다.

마루는 이 셋을 체인에 내장된 기능 세 개로 나눠 해결한다. 금액 노출은 Privacy 프리컴파일이 해결한다. 기업이 OKRW를 비공개 풀에 넣으면 이후 이체는 금액과 수취인이 암호화된 노트 단위로 기록되고, 직원 여러 명분 지급이 트랜잭션 한 건, 증명 한 개로 끝난다. 규제 증빙은 둘로 나뉜다. 지급 전 검사는 PCL(Programmable Compliance Layer)이 맡는다. 신원 확인과 한도 정책이 컨트랙트 코드가 아니라 체인 레벨에 등록되므로 지급 컨트랙트를 다시 배포하지 않고 정책을 바꿀 수 있다. 지급 후 복원은 Privacy 프리컴파일이 맡는다. 비공개 이체마다 감사 키로 암호화된 사본이 강제로 붙어 감사자만 사후에 복원한다. 정산 비용은 OKRW가 해결한다. 원화에 1:1로 고정되도록 설계된 네이티브 통화라 급여 자산과 가스 자산이 하나이고, 지급 컨트랙트에 ERC-20 allowance와 transferFrom 계층이 없다. 직원의 출금은 EIP-712 서명을 받아 은행이 대신 실행할 수 있게 설계되어 있어(문서 기준) 직원이 가스용 자산을 따로 준비할 필요가 없다. 이 문서는 이 세 기능을 급여 지급 한 흐름 안에서 이어 붙이는 방법을 다룬다.
...
```mermaid
%%{init: {'theme': 'dark', 'sequence': {'actorMargin': 140, 'width': 130, 'messageMargin': 40, 'noteMargin': 12}}}%%
sequenceDiagram
...
| 기업                    | 본다      | 전부. self-view 사본 | 전부. self-view 사본                      | 전부. 급여 대장               | 본다. 다만 어느 출금이 자기 급여 노트에서 나왔는지는 연결하지 못한다 |
| 은행                    | 본다      | 못 본다           | 못 본다                                      | 안다. KYC 기록                | 본다. relayer로 요청을 받는다                    |
| 감사자                  | 본다      | 전부. 감사 사본   | 전부. 감사 사본                              | 못 본다. 실명 대응은 체인 밖  | 본다                                             |

정책 거절은 실패한 트랜잭션으로 공개되어 누구나 거절 사실을 본다. 표준 receipt에는 status, gasUsed, logs만 있고 revert data는 없으므로, 어떤 에러였는지는 `eth_call`이나 debug trace로 확인한다. 이 가이드의 에러 이름도 callTracer로 뽑았다(`evidence/testnet-pcl/evidence/pcl-proxy-tx-trace.txt`). 예치 총액과 출금액 열이 전부 본다인 것이 이 설계의 경계다.
...
은행이 드는 키는 PCL admin, 프록시 admin, attester, relayer 넷이다. 넷을 서로 다른 지갑으로 분리하고 앞의 둘은 멀티시그로 둔다. PCL admin이 새면 `changeContractPolicies` 한 번으로 EAS 규칙 자체를 뺄 수 있고, attester가 새면 가짜 attestation이 OKRW_EAS 템플릿의 금액 면제를 얻는다. 둘은 서로를 보완하는 방어층이 아니라 각각 단독으로 정책 우회를 만든다. 그래서 분리 보관하고 정책 변경 이벤트와 attestation 발급 이벤트를 따로 감시한다. 은행 권한과 독립된 방어층은 마루 policyAdmin의 전역 정책과 Privacy 주소 정책뿐이다. 감사 키는 은행이 들지 않는다. 은행이 감사 키까지 들면 급여를 전부 볼 수 있는 단일 주체가 되어 비공개 풀을 쓰는 이유가 사라진다.
...
재시도는 멱등해야 한다. 실패한 트랜잭션은 nullifier를 기록하지 않으므로, 증명이 참조한 머클 루트가 아직 유효하고 위임 서명의 deadline이 지나지 않았을 때만 같은 payload를 다시 보낸다. 아니면 최신 루트로 다시 준비하거나 다시 서명받는다. 성공한 것을 다시 보내면 nullifier가 거절한다. 은행 백엔드는 입력 노트의 nullifier를 멱등 키로 삼는다.
